...
public class CommentBean implements CommentBeanI {
   ...
   /* public List<CommentTag> getAllCommentsForTest(Test exam) {
        javax.persistence.Query query = entityManager.createQuery("SELECT i FROM CommentTag i WHERE i.exam=:param");
...
/*    public List<CommentTag> getAllCommentsForTestViaCriteria(Test exam) {
        ...
        CriteriaQuery<CommentTag> criteriaQuery = criteriaBuilder.createQuery(CommentTag.class);
        Root<CommentTag> comment = criteriaQuery.from(CommentTag.class);
        criteriaQuery.select(comment).where(criteriaBuilder.equal(comment.get("exam"), exam));
        TypedQuery<CommentTag> query = entityManager.createQuery(criteriaQuery);
        return query.getResultList();
    }*/

    public List<Comment> getAllNonAssignedComments() {
        javax.persistence.Query query = entityManager.createQuery("SELECT i FROM Comment i WHERE i.exam is null");
        List<Comment> list = query.getResultList();
        return list;
    }

    public List<Comment> getAllCommentsForPerson(Person person) {
        javax.persistence.Query query = entityManager.createQuery("SELECT i FROM Comment i WHERE i.user=:param");
    ...
    public List<Comment> getAllGeneralComments() {
        Query query = entityManager.createQuery("SELECT c FROM Comment c WHERE c.type=:param order by c.id");
